...
const ProfileForm = () => {
  const [userFormData, setUserFormData] = useState<User>({ username: '', email: '', password: '', resume: [] });
    ...
    const handleFormSubmit = async (event:FormEvent) => {
        ...
        try {
            ...
            setUserFormData(
              { username: '', email: '', password: '', resume: [] }
            );
        }
        catch (err) {
            console.error(err);
    ...
    return (<div>
      ...
      <form className="flex-row justify-center justify-space-between-md align-center" onSubmit={handleFormSubmit}>
        <div className="col-12 col-lg-9">
          <input placeholder="Add your profile Username..." value={userFormData.username || ''} className="form-input w-100" onChange={handleInputChange}/>
        </div>
        <div className="col-12 col-lg-9">
          <input placeholder="Add your profile Email..." value={userFormData.email || ''} className="form-input w-100" onChange={handleInputChange}/>
        </div>
        <div className="col-12 col-lg-9">
          <input placeholder="Add your profile Password..." value={userFormData.password || ''} className="form-input w-100" onChange={handleInputChange}/>
        </div>
        ...
        {error && (<div className="col-12 my-3 bg-danger text-white p-3">
